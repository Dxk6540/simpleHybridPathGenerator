...
%             
% %             scatter3(toolContactPtSeq(1:cRtcpCylinder.drawStep_:end,1), toolContactPtSeq(1:cRtcpCylinder.drawStep_:end,2), toolContactPtSeq(1:cRtcpCylinder.drawStep_:end,3),2)
% %             hold on
% %             scatter3(toolTipPtSeq(1:cRtcpCylinder.drawStep_:end,1), toolTipPtSeq(1:cRtcpCylinder.drawStep_:end,2), toolTipPtSeq(1:cRtcpCylinder.drawStep_:end,3),2)
% %             hold on
%             ax = gca;
%             cRtcpCylinder.drawTools(ax, toolTipPtSeq, toolAxisSeq, cRtcpCylinder.drawStep_);
%             axis equal
%             hold on
% 
...
%         function pathSeq = genMachiningPath(~, startCenter, tol, wpHeight, lyrThickness, toolRadiu, wallOffset, zOffset, side, startIdx)
        function pathSeq = genMachiningPath(~, geoParam, procParam, wallOffset, side, startIdx)
%             geoParam.profileRadiu = 20;
%             geoParam.height = 20;
%             geoParam.center = [0,20,0];
% 
...
%             
            ...
            for lyrIdx = startIdx:lyrNum
                z = wpHeight - lyrIdx * lyrHeight + zOffset;
                vaseRadius = cRtcpCylinder.genVaseRadius(z); 
                fcNorm2 = vase.getVaseNormal(z, side);
                toolAxis2d = [side * sin(rollAgl),cos(rollAgl)];
                mtRadiu = vaseRadius + side * wallOffset;                            
                x = cos(0) * mtRadiu + startCenter(1);
            ...
            for lyrIdx = lyrNum:-1:startIdx
                ...
                vaseRadius = cRtcpCylinder.genVaseRadius(z); 
                fcNorm2 = cRtcpCylinder.getVaseNormal(z, side);
                toolAxis2d = [side * sin(rollAgl),cos(rollAgl)];
            ...
            toolContactPtSeq = cell2mat(toolContactPtSeq);
            toolTipPtSeq = cell2mat(toolTipPtSeq);
            toolAxisSeq = cell2mat(toolAxisSeq);
            drawStep_ = 5;
            plot3(toolContactPtSeq(1:cRtcpCylinder.drawStep_:end,1), toolContactPtSeq(1:cRtcpCylinder.drawStep_:end,2), toolContactPtSeq(1:cRtcpCylinder.drawStep_:end,3))
            hold on
            drawStep_ = 5;
            plot3(toolTipPtSeq(1:cRtcpCylinder.drawStep_:end,1), toolTipPtSeq(1:cRtcpCylinder.drawStep_:end,2), toolTipPtSeq(1:cRtcpCylinder.drawStep_:end,3))
            hold on
            ax = gca;
            cRtcpCylinder.drawTools(ax, toolTipPtSeq, toolAxisSeq, cRtcpCylinder.drawStep_);
            axis equal
