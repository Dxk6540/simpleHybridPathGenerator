...
% transitionDist=kernelNum*tStep*max(v1,v2);
...
%     plot(t_v_pos_d_seq(:,1),t_v_pos_d_seq(:,2),'Marker','*','Color','blue',LineWidth=1);
    plot(t_v_pos_d_seq(:,1),t_v_pos_d_seq(:,2),'Marker','*','Color','blue');
    xlabel('time [s]');
    ylabel('feed rate [mm/s]');
    title('v - t');
    subplot(1,3,2)
    plot(t_v_pos_d_seq(:,1),t_v_pos_d_seq(:,6),'Marker','*','Color','blue');
    xlabel('time [s]');
    ylabel('distance [mm]');
    title('d - t');
    subplot(1,3,3)
    plot(t_v_pos_d_seq(:,6),t_v_pos_d_seq(:,2),'Marker','*','Color','blue');
    hold on
    plot(pos_v_d_seq(:,5),pos_v_d_seq(:,4),'Marker','o');
    xlabel('distance [mm]');
    ylabel('feed rate [mm/s]');
    title('v - d');
