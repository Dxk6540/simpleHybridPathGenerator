function J = sigmoid_fit(Input_x,Fitting_par,Reference_y,prvious_v)
...
k = 3 * [1,1,1,1]; % cost function weight

change_of_par = abs(prvious_v - Fitting_par);
cost_par_change = k .* change_of_par;

% J = 60*norm(Error_y,1)/size(Error_y,2) + norm(cost_par_change,1)/4;
J = sqrt(norm(Error_y,2)/size(Error_y,2));
