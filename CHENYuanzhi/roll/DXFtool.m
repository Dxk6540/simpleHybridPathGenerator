        ...
        function plot(dxf)
            ...
            for i = 1:dxf.ne
                ...
                switch dxf.entities(i).name
                    case 'POINT'
                        [dxf.entities(i).handle,dxf.entities(i).points,dxf.entities(i).vertex] = plot_point(dxf.entities(i).point,line_col);
                    case 'LINE'
                        [dxf.entities(i).handle,dxf.entities(i).points,dxf.entities(i).vertex] = plot_line(dxf.entities(i).line,line_col);
                    case 'LWPOLYLINE'
                        [dxf.entities(i).handle,dxf.entities(i).points,dxf.entities(i).vertex] = plot_poly(dxf.entities(i).poly,line_col,hatch_col,dxf.entities(i).closed);
                    case 'CIRCLE'
                        [dxf.entities(i).handle,dxf.entities(i).points,dxf.entities(i).vertex] = plot_circle(dxf.entities(i).circle,dxf.divisions,line_col);
                    case 'ARC'
                        [dxf.entities(i).handle,dxf.entities(i).points,dxf.entities(i).vertex] = plot_arc(dxf.entities(i).arc,dxf.divisions,line_col);
                    case 'ELLIPSE'
                        [dxf.entities(i).handle,dxf.entities(i).points,dxf.entities(i).vertex] = plot_ellipse(dxf.entities(i).ellipse,dxf.divisions,line_col,hatch_col,dxf.entities(i).closed);
                    case 'SPLINE'
                        [dxf.entities(i).handle,dxf.entities(i).points,dxf.entities(i).vertex] = plot_spline(dxf.entities(i).spline,line_col);
                end
                dxf.points=[dxf.points;dxf.entities(i).points];
...
function [h,points,vertex] = plot_poly(poly,col,col2,isclosed)
% plot polygons incl. bulges by vertices (X,Y)
    ...
    points=[poly(1,:);poly(end,:)];
end

function [h,points,vertex] = plot_circle(circle,div,col)
% plot circles: (X Center,Y Center,Radius)
    
    x = circle(1);
    y = circle(2);
    r = circle(3);
    div=max(div,floor(2*pi*r/0.01));
    ...
    points=[X(1),Y(1);X(end),Y(end)];
end

function [h,points,vertex] = plot_arc(arc,div,col)
% plot arcs: (X Center,Y Center,Radius,Start angle,End angle)
    ...
    div=max(div,floor((a2-a1)/180*pi*r/0.01));
...
function [h,points,vertex] = plot_ellipse(ellipse,div,line_col,hatch_col,isclosed)
...
function [h,points,vertex] = plot_point(point,col)
% plot points
...
function [h,points,vertex] = plot_line(line,col)
...
function [h,points,vertex] = plot_spline(sp,col)
% plot splines as piecewise linear poly line (very crude) 

    poly = [sp.X sp.Y];
    [h,points,vertex] = plot_poly(poly,col,'',0);
...
% calculate n points on arc (bulge) described by 
% starting point P1, ending point P2 and bulge b.
